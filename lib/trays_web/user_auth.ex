defmodule TraysWeb.UserAuth do
  use TraysWeb, :verified_routes
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      TraysWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  defp maybe_reissue_user_session_token(conn, user, token_inserted_at) do
    token_age = DateTime.utc_now(:second) |> DateTime.diff(token_inserted_at, :day)
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      TraysWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule TraysWeb.PageLive do
        use TraysWeb, :live_view

        on_mount {TraysWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{TraysWeb.UserAuth, :require_authenticated}] do
